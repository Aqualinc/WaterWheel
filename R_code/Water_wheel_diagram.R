...
#This script prepares a wheel of water diagram from an array of input values
...
indicator_values      <-  as.numeric(unlist(indicator_array[8,]))                 #These are the actual values of the indicators
...
for (indicator in c(1:length(indicators))) {                                      #scale to fit within 0-100 with class divisions at 25,50 and 75
  indicator_limits    <-as.numeric(unlist(indicator_array[3:7,indicator]))
